...
def run_mcp_command(method, params=None, show_request=True, show_raw_response=False):
    ...
    if show_request:
        ...
        syntax = Syntax(json.dumps(request, indent=2), "json", theme="monokai", line_numbers=False)
            ...
            for line in output.split('\n'):
                if line.strip():
                    try:
                        ...
                        if show_raw_response:
                            console.print(f"[red]Failed to parse line[/]")
...
def display_tools(tools, max_tools=30):
    ...
    table = Table(title="GitHub MCP Server Tools", show_header=True, header_style="bold magenta")
    ...
    display_tools = (sorted(repo_tools, key=lambda x: x["name"]) + 
...
def extract_repos_from_response(response_data):
    ...
    if isinstance(response_data, list):
        ...
        if all(isinstance(item, dict) and "full_name" in item for item in response_data[:3]):
        ...
        elif "repositories" in response_data and isinstance(response_data["repositories"], list):
                ...
                if isinstance(item, dict) and "text" in item:
                    try:
                        ...
                        if isinstance(data, dict):
                            ...
                            # If it's directly a list, check if it looks like repositories
                            if all(isinstance(repo, dict) and "full_name" in repo for repo in data[:3]):
                                return data
                    except:
                        pass
...
def get_tools_list(show_request=False):
    ...
    valid_result = next((r for r in tools_result if "result" in r and "tools" in r["result"]), None)
    if not valid_result:
...
def search_popular_repos():
    ...
    search_tool = next((t for t in tools_list if t["name"] == "search_repositories"), None)
    ...
    search_result = run_mcp_command("tools/call", {
    ...
    if search_result:
        ...
        if valid_search and "result" in valid_search:
            ...
            if items_json:
                ...
                console.print("[bold yellow]No repositories extracted from the response[/]")
        else:
...
def get_readme_content():
    ...
    file_result = run_mcp_command("tools/call", {
    ...
    if file_result:
        ...
        if valid_file and "result" in valid_file:
                        ...
                        try:
                            ...
                            if content_match:
                                ...
                                try:
                                    ...
                                    console.print("[dim]Successfully extracted README content using regex[/]")
                                    break
                                except Exception as e:
                                    console.print(f"[dim]Base64 decode error: {str(e)[:100]}[/]")
                        except Exception as e:
                            console.print(f"[dim]Regex extraction error: {str(e)[:100]}[/]")
            
            # If regex approach failed, try the previous approaches
            if not decoded_content:
                console.print("[dim]Regex approach failed for README, trying JSON parsing methods...[/]")
                ...
                # Format 2: Result is a list of content items with text field
                    ...
                    for item in result:
                        if isinstance(item, dict) and "text" in item:
                            try:
                                # Extract the content JSON from the text
                                # Fix: Remove newlines that might be breaking JSON parsing
                                ...
                                console.print(f"[dim]README parsing error: {str(e)[:100]}[/]")
                
                # Format 3: Content array in result
                elif isinstance(result, dict) and "content" in result and isinstance(result["content"], list):
                    for item in result["content"]:
                        if isinstance(item, dict) and "text" in item:
                            try:
                                ...
                                console.print(f"[dim]README format 3 parsing error: {str(e)[:100]}[/]")
            ...
            if decoded_content:
                ...
                preview = decoded_content[:800] + "..." if len(decoded_content) > 800 else decoded_content
                console.print("[bold green]README.md from facebook/react:[/]")
                console.print(Panel(preview, border_style="green", title="README.md Preview"))
            else:
                console.print("[bold yellow]Could not extract README content from response[/]")
...
def list_org_repos():
    ...
    repos_result = run_mcp_command("tools/call", {
        "name": "search_repositories",
        "arguments": {
    ...
    if repos_result:
        ...
        if valid_repos and "result" in valid_repos:
            ...
            if repos_json and len(repos_json) > 0:
                ...
                console.print(f"[dim]Trying alternative query format...[/]")
                
                alt_repos_result = run_mcp_command("tools/call", {
                    "name": "search_repositories",
                    "arguments": {
                        "query": f"user:{org_name}",
                        "perPage": 5
                    }
                }, show_raw_response=False)
                
                if alt_repos_result:
                    alt_valid_repos = next((r for r in alt_repos_result if "result" in r), None)
                    
                    if alt_valid_repos and "result" in alt_valid_repos:
                        alt_repos_json = extract_repos_from_response(alt_valid_repos["result"])
                        ...
                        if alt_repos_json and len(alt_repos_json) > 0:
                            ...
                            console.print(f"[bold yellow]No repositories found for {org_name} with alternative query[/]")
        else:
                ...
                try:
                    ...
                    if content_match:
                        ...
                        try:
                            ...
                            console.print("[dim]Successfully extracted content using regex[/]")
                            break
                        except Exception as e:
                            console.print(f"[dim]Base64 decode error: {str(e)[:100]}[/]")
                except Exception as e:
        ...
        # Format 2: Result is a list of content items with text field
            ...
            for item in result:
                if isinstance(item, dict) and "text" in item:
                    try:
                        # Extract the content JSON from the text
                        # Remove newlines and other whitespace that might break JSON parsing
                        text = item["text"].replace('\n', '').replace('\\n', '').strip()
                        content_json = json.loads(text)
                        ...
                        if isinstance(item["text"], str) and (item["text"].startswith("{") or item["text"].startswith("[")):
                            try:
                                # If it's JSON, format it nicely
                                file_content = json.dumps(json.loads(item["text"]), indent=2)
                                break
                            except:
...
def display_file_content(file_path, owner, repo, file_content):
    ...
    preview_length = 2000
    preview = file_content[:preview_length] + "..." if len(file_content) > preview_length else file_content
...
def fetch_specific_repo_file():
    ...
    if file_content:
        display_file_content("README.md", "VectorInstitute", "vector-inference", file_content)
...
def read_api_docs():
    ...
    if docs_result:
        ...
        if valid_file and "result" in valid_file and "message" not in valid_file:
            console.print("[bold green]Found docs directory. Looking for API documentation files...[/]")
        else:
            # If docs directory doesn't exist, try common files at root
            console.print("[dim]No docs directory found. Looking for API documentation at repository root...[/]")
            possible_doc_paths = ["API.md", "api.md", "USAGE.md", "usage.md", "README.md"]
    ...
    for path in possible_doc_paths:
        ...
        if file_result:
            valid_file = next((r for r in file_result if "result" in r and "message" not in r), None)
            if valid_file and "result" in valid_file:
    ...
    if not api_doc_content:
        console.print("[dim]No specific API documentation found. Trying README.md...[/]")
        ...
        if readme_result:
            ...
            if api_doc_content:
                ...
                console.print("[bold green]Using README.md as it may contain API information[/]")
    ...
    if api_doc_content:
        ...
        api_keywords = ["api", "function", "method", "class", "parameter", "usage", "example"]
        has_api_content = any(keyword in api_doc_content.lower() for keyword in api_keywords)
        
        if not has_api_content:
            console.print("[bold yellow]Note: This file might not contain detailed API documentation.[/]")
            
            # Try to find a Python example file as another approach
            console.print("[dim]Looking for Python example files...[/]")
            example_paths = ["examples/example.py", "example.py", "examples/demo.py", "demo.py"]
            ...
            for path in example_paths:
                ...
                if example_result:
                    ...
                    if example_content:
                        console.print("[bold green]Found an example file that might help with API usage:[/]")
                        display_file_content(path, owner, repo, example_content)
                        break
    else:
        console.print("[bold red]Could not find API documentation for this repository.[/]")
        
        # As a last resort, try to find a Python file that might show API usage
        console.print("[dim]Trying to find a Python source file that might demonstrate API usage...[/]")
        source_paths = ["src/main.py", "src/__init__.py", "cyclops/__init__.py", "main.py"]
        ...
        for path in source_paths:
            ...
            if source_result:
                ...
                if source_content:
                    console.print("[bold green]Found a source file that might help understand the API:[/]")
...
def parse_arguments():
    ...
    parser.add_argument("--list_demos", action="store_true", help="List all available demos")
    parser.add_argument("--display_tools", action="store_true", help="Show available GitHub MCP tools")
    parser.add_argument("--search_repos", action="store_true", help="Search for popular GitHub repositories")
    parser.add_argument("--get_readme", action="store_true", help="Fetch and display README from a repository")
    parser.add_argument("--list_org_repos", action="store_true", help="List repositories from a specific organization")
    parser.add_argument("--get_repo_file", action="store_true", help="Fetch a specific file from a repository")
    parser.add_argument("--get_api_docs", action="store_true", help="Find and display API documentation")
    parser.add_argument("--run_all", action="store_true", help="Run all demos in sequence")
    
    return parser.parse_args()

def main():
    """Main function to run the GitHub MCP server demo"""
